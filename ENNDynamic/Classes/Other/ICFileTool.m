...
+(BOOL)saveFileToTemporaryDirectory:(NSString *)fileName fileData:(NSData *)fileData {
   __block BOOL  write = NO;
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSData* attachmentData = fileData;
        NSString *tmpDirectory = NSTemporaryDirectory();
        NSString *filePath=[tmpDirectory stringByAppendingPathComponent:fileName];
        write = [attachmentData writeToFile:filePath atomically:YES];
    });
    return write;
}
