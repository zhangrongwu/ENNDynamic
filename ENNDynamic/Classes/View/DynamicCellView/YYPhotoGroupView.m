...
@interface YYPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate, ICActionSheetViewDelegate>
...
@property (nonatomic, strong) NSString *urlSring;
...
- (void)longPress:(UILongPressGestureRecognizer*)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        //    activityViewController.restorationIdentifier = @"activity";
        ...
        self.saveImage = tile.imageView.image;
        
        CIDetector *detector = [CIDetector detectorOfType:CIDetectorTypeQRCode context:nil options:@{CIDetectorAccuracy:CIDetectorAccuracyHigh}];
        NSArray *features = [detector featuresInImage:[CIImage imageWithCGImage:tile.imageView.image.CGImage]];
        if (features.count >= 1) {
            CIQRCodeFeature *feature = [features firstObject];
            self.urlSring = feature.messageString;
        }  else{
            self.urlSring = @"";
        }
        _actionSheet = nil;
        if (!_actionSheet) {
            if(![ICTools stringEmpty:self.urlSring] && self.fromViewController) {
                self.actionSheet = [[ICActionSheetView alloc] initWithTitles:@[NSLocalizedString(@"savePhoto", nil), NSLocalizedString(@"ExtractQRcode", nil)] delegate:self cancelButtonTitle:@"取消"];
            } else {
                self.actionSheet = [[ICActionSheetView alloc] initWithTitles:@[NSLocalizedString(@"savePhoto", nil)] delegate:self cancelButtonTitle:@"取消"];
            }
        }
...
- (void)ActionSheetView:(ICActionSheetView *)actionSheetView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if(![ICTools stringEmpty:self.urlSring]) {
        if (buttonIndex == 1) {
            [self.fromViewController pushCustomViewControllerURL:self.urlSring fromViewController:self.fromViewController animated:YES];
            [self dismiss];
        }
    }
}
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error) {
        [MBProgressHUD showError:@"保存图片失败" toView:self];
    } else {
        [MBProgressHUD showSuccess:@"保存图片成功" toView:self];
    }
}
